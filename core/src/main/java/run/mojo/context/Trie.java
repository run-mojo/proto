...
final class Trie<K,V> {
   ...
   */
  public Trie<K,V> put(K key, V value) {
    if (root == null) {
      return new Trie<K,V>(new Leaf<K,V>(key, value));
    } else {
      return new Trie<K,V>(root.put(key, value, key.hashCode(), 0));
    }
  }
