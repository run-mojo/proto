...
/**  */
class PackageModel internal constructor(
    val type: Any?,
    val name: String,
    val simpleName: String,
...
fun testIt(model: WireModel) {
    when (model) {
        is InvalidModel -> model.type
    }
}


/**  */
abstract class WireModel constructor(open val javaKind: JavaKind, open val protoType: ProtoType) {
...
data class InvalidModel(
    ...
    val type: String
) : WireModel(JavaKind.INVALID, ProtoType.get("!"))

object NothingModel : WireModel(JavaKind.VOID, ProtoType.get("void"))


/**  */
data class PrimitiveModel(
    val javaType: JavaKind
) : WireModel(javaType, asProtoType(javaType)) {
    ...
    companion object {
        ...
        private fun asProtoType(javaType: JavaKind): ProtoType {
            when (javaType) {
                JavaKind.BOOL,
                JavaKind.BOXED_BOOL -> return ProtoType.BOOL

                JavaKind.BYTE,
                JavaKind.BOXED_BYTE,
                JavaKind.SHORT,
                JavaKind.BOXED_SHORT,
                JavaKind.CHAR,
                JavaKind.BOXED_CHAR,
                JavaKind.INT,
                JavaKind.BOXED_INT -> return ProtoType.INT32
...
/**  */
data class StringModel(var type: Any) : WireModel(JavaKind.STRING, ProtoType.STRING) {
...
/**  */
data class BytesModel(var type: Any) : WireModel(JavaKind.BYTES, ProtoType.BYTES) {
...
data class ListModel(
    override val javaKind: JavaKind,
    val component: WireModel
) : WireModel(javaKind, component.protoType) {
    ...
    override fun toTypeName(boxed: Boolean): TypeName {
        return when (javaKind) {
            ...
            JavaKind.LIST -> ParameterizedTypeName.get(
                ClassName.get(List::class.java),
                component.toTypeName(boxed)
            )

            JavaKind.SET -> ParameterizedTypeName.get(
                ClassName.get(Set::class.java),
                component.toTypeName(boxed)
            )

            JavaKind.QUEUE -> ParameterizedTypeName.get(
                ClassName.get(Queue::class.java),
                component.toTypeName(boxed)
            )
...
/**  */
data class MapModel(val key: WireModel, val value: WireModel) : WireModel(
